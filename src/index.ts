...
const uppercaseFirstLetter = (s: string) => `${s.toUpperCase()[0]}${s.substring(1)}`;
...
const generateApi = async (outputPath: string, service: string, openApi: OpenApi) => {
    const content = `
...
`;
    await fs.writeFile(`${outputPath}/src/${service}/api.ts`, content, { encoding: 'utf-8' });
};
...
const generateRoute = (path: string, method: string, route: any): string => {
    return `app.${method}("${path}", (req, res) => {
        res.send({ msg: "${Buffer.from(Math.random().toString().substring(2, 6)).toString("base64")}"});
    });`;
};

const copyOpenApi = async (outputPath: string, service: string, spec: string) => {
    const openApi = JSON.parse(await fs.readFile(spec, {encoding: 'utf-8'}));
    openApi.schemes = ["http"];
    openApi.host = "localhost:3000";
    const content = JSON.stringify(openApi, null, 2);
    await fs.writeFile(`${outputPath}/src/${service}/openapi.json`, content, {encoding: 'utf-8'});
};

await generateServiceMocks("output", ["test/communicationservicesrooms.json"]);
